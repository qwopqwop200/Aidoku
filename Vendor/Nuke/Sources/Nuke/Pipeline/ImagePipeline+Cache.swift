...
extension ImagePipeline.Cache {
    ...
    public func makeDataCacheKey(for request: ImageRequest) -> String {
        ...
        // Invalidate old URL-only records within the same bounded disk cache.
        return "aidoku-image-data-v2-\(request.imageID ?? "")\(request.thumbnail?.identifier ?? "")\(ImageProcessors.Composition(request.processors).identifier)"
    }
