...
M3Result  InitMemory  (IM3Runtime io_runtime, IM3Module i_module)
{
    ...
    if (not i_module->memoryImported)
    {
        ...
        io_runtime->memory.maxPages = maxPages;
...
M3Result  ResizeMemory  (IM3Runtime io_runtime, u32 i_numPages)
{
    ...
    {
        _throwif (m3Err_wasmMemoryOverflow, memory->pageSize == 0 ||
                  numPagesToAlloc > (SIZE_MAX - sizeof (M3MemoryHeader)) / memory->pageSize);
        size_t numPageBytes = (size_t) numPagesToAlloc * memory->pageSize;
        ...
        size_t numPreviousBytes = memory->mallocated
            ? memory->mallocated->length + sizeof (M3MemoryHeader) : 0;
...
_           (ReadLEB_u32 (& numElements, & bytes, end));
            ...
            {
                IM3Function * table = m3_ReallocArray (IM3Function, io_module->table0, endElement, io_module->table0Size);
                _throwifnull (table);
                io_module->table0 = table;
...
{
    return (i_runtime && i_runtime->memory.mallocated)
        ? (u32) i_runtime->memory.mallocated->length : 0;
}
