...
void internal_itoa(int n, char s[], int radix)
{
    ...
    sign = n;
    // Unsigned subtraction also represents the magnitude of INT_MIN.
    unsigned magnitude = n < 0 ? 0u - (unsigned)n : (unsigned)n;
    i = 0;
    do {
        s[i++] = HEXDIGITS[magnitude % (unsigned)radix];
    } while ((magnitude /= (unsigned)radix) > 0);
...
m3ApiRawFunction(m3_libc_printf)
{
    ...
    size_t fmt_available = m3_GetMemorySize(runtime) - m3ApiPtrToOffset(i_fmt);
    size_t fmt_len = strnlen(i_fmt, fmt_available);
    if (fmt_len == fmt_available) m3ApiTrap(m3Err_trapOutOfBoundsMemoryAccess);
    ...
    while ((ch = *i_fmt++)) {
        ...
        ch = *i_fmt++;
        if (!ch) break; // A trailing percent must not walk past the terminator.
        switch (ch) {
            case 'c': {
            ...
            case 's': {
                ...
                if (m3ApiIsNullPtr(string_temp)) {
                    ...
                    m3ApiCheckMem(string_temp, 1);
                    size_t available = m3_GetMemorySize(runtime) - m3ApiPtrToOffset(string_temp);
                    string_len = strnlen(string_temp, available);
                    if (string_len == available) m3ApiTrap(m3Err_trapOutOfBoundsMemoryAccess);
                }
