...
m3ApiRawFunction(m3_wasi_generic_args_sizes_get)
{
    ...
    for (u32 i = 0; i < context->argc; ++i)
    {
        ...
        if (length >= UINT32_MAX - buf_len) { m3ApiReturn(UVWASI_EOVERFLOW); }
        buf_len += length + 1;
    }
...
m3ApiRawFunction(m3_wasi_generic_environ_get)
{
    ...
    m3ApiCheckMem(env,      (uint64_t)env_count * sizeof(uint32_t));
...
m3ApiRawFunction(m3_wasi_generic_environ_sizes_get)
{
    ...
    uvwasi_size_t count = 0;
    uvwasi_size_t buf_size = 0;
...
#else
    if (iovs_len > 128) m3ApiReturn(UVWASI_EINVAL);
    uvwasi_ciovec_t  iovs[128];
#endif
    uvwasi_size_t num_written = 0;
    uvwasi_errno_t ret;
...
m3ApiRawFunction(m3_wasi_generic_poll_oneoff)
{
    ...
    m3ApiCheckMem(in,       (uint64_t)nsubscriptions * sizeof(uvwasi_subscription_t));
    m3ApiCheckMem(out,      (uint64_t)nsubscriptions * sizeof(uvwasi_event_t));
