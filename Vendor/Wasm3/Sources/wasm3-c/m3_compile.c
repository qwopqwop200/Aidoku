...
{
    IM3CodePage savedPage = NULL;
_try {
    u32 targetCount;
_   (ReadLEB_u32 (& targetCount, & o->wasm, o->wasmEnd));

    // Each label (including the default) requires at least one input byte.
    // Validate before sizing a code page, including its two linking slots.
    _throwif(m3Err_wasmMalformed, targetCount > UINT32_MAX - 6 ||
             (size_t) targetCount >= (size_t) (o->wasmEnd - o->wasm));
...
_       (Pop (o));
    ...
    u32 frameEnd = (u32) topSlot + ((u32) numArgs + numRets) * c_ioSlotCount;
    _throwif(m3Err_functionStackOverflow, frameEnd > d_m3MaxFunctionSlots);
    if (frameEnd) TouchSlot (o, frameEnd - 1);
    u16 argTop = (u16) frameEnd;
...
_   (CompileBlockStatements (o));
    ...
    // Publish only after every allocation succeeds so a retry cannot execute
    // partially initialized code with a missing constants buffer.
