...
m3ApiRawFunction(m3_wasi_generic_args_get)
{
    ...
    m3ApiCheckMem(argv, (uint64_t)context->argc * sizeof(uint32_t));
...
m3ApiRawFunction(m3_wasi_generic_args_sizes_get)
{
    ...
    for (u32 i = 0; i < context->argc; ++i)
    {
        size_t length = strlen(context->argv[i]);
        if (length >= UINT32_MAX - buf_len) { m3ApiReturn(__WASI_ERRNO_OVERFLOW); }
        buf_len += length + 1;
    }
...
m3ApiRawFunction(m3_wasi_generic_fd_prestat_dir_name)
{
    ...
    if (path_len < slen) { m3ApiReturn(__WASI_ERRNO_NAMETOOLONG); }
    memcpy(path, preopen[fd].path, slen);
    m3ApiReturn(__WASI_ERRNO_SUCCESS);
}
...
m3ApiRawFunction(m3_wasi_generic_fd_prestat_get)
{
    ...
    m3ApiWriteMem32(buf+4, strlen(preopen[fd].path));
    m3ApiReturn(__WASI_ERRNO_SUCCESS);
}
...
m3ApiRawFunction(m3_wasi_generic_fd_fdstat_set_flags)
{
    ...
    if (current < 0) { m3ApiReturn(errno_to_wasi(errno)); }
    if (flags & ~(__WASI_FDFLAGS_APPEND | __WASI_FDFLAGS_NONBLOCK)) {
        m3ApiReturn(__WASI_ERRNO_NOTSUP);
    }
    int replacement = (current & ~(O_APPEND | O_NONBLOCK)) |
        ((flags & __WASI_FDFLAGS_APPEND) ? O_APPEND : 0) |
        ((flags & __WASI_FDFLAGS_NONBLOCK) ? O_NONBLOCK : 0);
    if (fcntl(fd, F_SETFL, replacement) < 0) { m3ApiReturn(errno_to_wasi(errno)); }
    m3ApiReturn(__WASI_ERRNO_SUCCESS);
#endif
}
...
m3ApiRawFunction(m3_wasi_generic_path_open)
{
    ...
    int mode = 0644;
    // Open the directory only for this operation; repeated module linking must
    // not leak process-wide preopen descriptors.
    int directory_fd = open(preopen[dirfd].real_path, O_RDONLY);
    if (directory_fd < 0) { m3ApiReturn(errno_to_wasi(errno)); }
    int host_fd = openat(directory_fd, host_path, flags, mode);
...
m3ApiRawFunction(m3_wasi_generic_fd_read)
{
    ...
    if (iovs_len > 1024) { m3ApiReturn(__WASI_ERRNO_INVAL); }
...
m3ApiRawFunction(m3_wasi_generic_fd_close)
{
    ...
    int ret = close(fd);
    m3ApiReturn(ret == 0 ? __WASI_ERRNO_SUCCESS : errno_to_wasi(errno));
}
...
#   if defined(__APPLE__) && (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR)
        if (SecRandomCopyBytes(kSecRandomDefault, reqlen, buf) != errSecSuccess) {
            m3ApiReturn(__WASI_ERRNO_IO);
        }
        retlen = reqlen;
#   else
