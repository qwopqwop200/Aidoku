...
size_t  SPrintArg  (char * o_string, size_t i_stringBufferSize, voidptr_t i_sp, u8 i_type)
{
    ...
    if (!i_stringBufferSize) return 0;
    * o_string = 0;
    ...
    return M3_MIN ((size_t) len, i_stringBufferSize - 1);
}
...
cstr_t  SPrintFunctionArgList  (IM3Function i_function, m3stack_t i_sp)
{
    ...
    ret = snprintf (s, e-s, "(");
    s += M3_MIN ((size_t) M3_MAX (0, ret), (size_t) (e-s-1));
...
cstr_t  SPrintFuncTypeSignature  (IM3FuncType i_funcType)
{
    ...
    for (u32 i = 0; i < i_funcType->numArgs; ++i)
    {
        ...
        strncat (string, GetTypeName (d_FuncArgType(i_funcType, i)), sizeof(string) - strlen(string) - 1);
    }
    ...
    for (u32 i = 0; i < i_funcType->numRets; ++i)
    {
        ...
        strncat (string, GetTypeName (d_FuncRetType(i_funcType, i)), sizeof(string) - strlen(string) - 1);
    }
...
d_m3Decoder (Entry)
{
    ...
    snprintf (o_string, 8*1024, "%s", m3_GetFunctionName(function));
}
...
d_m3Decoder  (BranchTable)
{
    ...
    char * end = o_string + 8*1024;
    o_string += snprintf (o_string, end-o_string, "slot: %" PRIu32 "; targets: ", slot);
...
//    IM3Function function = fetch2 (IM3Function);
    ...
    for (i32 i = 0; i < targets; ++i)
    {
        ...
        int wrote = snprintf (o_string, end-o_string, "%" PRIi32 "=%p, ", i, addr);
        if (wrote > 0) o_string += M3_MIN ((size_t) wrote, (size_t)(end-o_string-1));
    }

    pc_t addr = fetch (pc_t);
    snprintf (o_string, end-o_string, "def=%p ", addr);
}
