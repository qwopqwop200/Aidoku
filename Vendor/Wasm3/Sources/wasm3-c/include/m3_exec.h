...
d_m3Op  (Compile)
{
    ...
    if (not result)
    {
        ...
        * ((void**) --_pc) = (void*) (function->compiled);
        // Leave the lazy compile opcode intact on failure so retry stays safe.
        rewrite_op (op_Call);
        --_pc;
        nextOpDirect ();
    }
...
d_m3Op  (Entry)
{
    ...
    if (M3_LIKELY (runtime->callDepth < d_m3MaxCallDepth))
#else
    if (M3_LIKELY ((void *) (_sp + function->maxStackSlots) < _mem->maxStack && runtime->callDepth < d_m3MaxCallDepth))
#endif
    {
#if defined(DEBUG)
        function->hits++;
