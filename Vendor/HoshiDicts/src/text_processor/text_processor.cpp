...
std::u32string nfkc(const std::u32string& text) {
  ...
  if (utf8.size() > static_cast<size_t>(std::numeric_limits<utf8proc_ssize_t>::max())) {
    throw std::length_error("Dictionary text exceeds normalization limit");
  }
  utf8proc_uint8_t* out = nullptr;
  const auto length = utf8proc_map(reinterpret_cast<const utf8proc_uint8_t*>(utf8.data()), utf8.size(), &out,
      static_cast<utf8proc_option_t>(UTF8PROC_STABLE | UTF8PROC_COMPAT | UTF8PROC_COMPOSE));
  std::unique_ptr<utf8proc_uint8_t, decltype(&utf8proc_free)> normalized(out, &utf8proc_free);
  if (length < 0) {
    throw std::runtime_error(std::string("Dictionary normalization failed: ") + utf8proc_errmsg(length));
  }
  if (!normalized) {
    throw std::bad_alloc();
  }
  std::string result(reinterpret_cast<char*>(normalized.get()), static_cast<size_t>(length));
...
char32_t add_dakuten(char32_t kana) {
  ...
  std::unique_ptr<utf8proc_uint8_t, decltype(&utf8proc_free)> normalized(out, &utf8proc_free);
  if (!normalized) {
    throw std::runtime_error("Dictionary kana normalization failed");
  }
  std::u32string composed = utf8::utf8to32(std::string(reinterpret_cast<char*>(normalized.get())));
