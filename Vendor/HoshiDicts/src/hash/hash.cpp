#include "hash.hpp"

#include <xxh3.h>

#include <cstddef>
#include <cstdint>
#include <cstdlib>
#include <cstring>
#include <memory>
#include <limits>
#include <stdexcept>

#include "../memory/memory.hpp"

namespace hash {
linear::linear() : ptr_(std::make_unique<table>()) {};
linear::~linear() = default;

uint64_t linear::operator()(std::string_view key) const {
  if (!ptr_->table || ptr_->capacity == 0) return 0;
  uint64_t h = XXH3_64bits(key.data(), key.size());
  if (bloom_ && !bloom_->contains(h)) return 0;
  uint64_t pos = h % ptr_->capacity;
  for (uint32_t visited = 0; visited < ptr_->capacity; ++visited) {
    slot entry;
    std::memcpy(&entry, ptr_->table + pos * sizeof(slot), sizeof(entry));
    if (entry.hash == 0) return 0;
    if (entry.hash == h) return entry.offset;
    pos = (pos + 1) % ptr_->capacity;
  }
  return 0;
}

void linear::build_to_file(const std::vector<std::pair<uint64_t, uint64_t>>& hash_entries,
                           const std::filesystem::path& path) {
  if (hash_entries.size() > (uint64_t(std::numeric_limits<uint32_t>::max()) * 7 / 10))
    throw std::length_error("hash table exceeds supported capacity");
  ptr_->capacity = std::max<uint64_t>(hash_entries.size() * 10 / 7, 16);
  size_t file_size = sizeof(uint32_t) + ptr_->capacity * sizeof(slot);

  auto out = memory::map_rw(path, file_size);
  if (!out) {
    throw std::runtime_error("failed to create hash table");
  }

  std::memcpy(out.data, &ptr_->capacity, sizeof(uint32_t));
  ptr_->table = out.data + sizeof(uint32_t);
  std::memset(ptr_->table, 0, ptr_->capacity * sizeof(slot));
  for (const auto& he : hash_entries) {
    uint64_t h = he.first;
    uint64_t pos = h % ptr_->capacity;
    while (true) {
      slot entry;
      std::memcpy(&entry, ptr_->table + pos * sizeof(slot), sizeof(entry));
      if (entry.hash == 0) {
        entry = {.hash = h, .offset = he.second};
        std::memcpy(ptr_->table + pos * sizeof(slot), &entry, sizeof(entry));
        break;
      }
      pos = (pos + 1) % ptr_->capacity;
    }
  }
  memory::unmap(out);
  ptr_->table = nullptr;
  ptr_->capacity = 0;
}

bool linear::load(uint8_t* ptr, size_t size) {
  ptr_->capacity = 0;
  ptr_->table = nullptr;
  if (!ptr || size < sizeof(uint32_t)) return false;
  uint32_t capacity;
  std::memcpy(&capacity, ptr, sizeof(capacity));
  if (capacity == 0 || (size - sizeof(uint32_t)) / sizeof(slot) != capacity ||
      (size - sizeof(uint32_t)) % sizeof(slot) != 0) return false;
  ptr_->capacity = capacity;
  ptr_->table = ptr + sizeof(uint32_t);
  return true;
}
}
