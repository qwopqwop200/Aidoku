   ...
   struct to<CSV, T>
   {
      ...
      static void op(auto&& value, is_context auto&& ctx, B&& b, auto& ix)
      {
         if constexpr (check_layout(Opts) == rowwise) {
            ...
            // An empty map has neither columns nor rows.
            if (value.empty()) return;
            const auto row_count = value.begin()->second.size();
            for (const auto& [name, data] : value) {
         ...
         if constexpr (check_layout(Opts) == rowwise) {
            for_each<N>([&]<auto I>() {
               ...
               if constexpr (writable_array_t<value_type>) {
                  ...
                  const auto size = [&] {
                     if constexpr (fixed_array_value_t<std::decay_t<decltype(member)>>) {
                        return std::tuple_size_v<value_type>;
                     }
                     else {
                        return count ? member[0].size() : size_t{0};
                     }
                  }();
            ...
            for_each<N>([&]<auto I>() {
               decltype(auto) mem = [&]() -> decltype(auto) {
                  if constexpr (reflectable<T>) return get<I>(t);
                  else return get<I>(reflect<T>::values);
               }();
               const auto count = get_member(value, mem).size();
               if constexpr (I == 0) row_count = count;
               else if (count != row_count) ctx.error = error_code::syntax_error;
            });
            ...
            if constexpr (check_use_headers(Opts)) {
               for_each<N>([&]<auto I>() {
                  ...
                  if constexpr (fixed_array_value_t<X>) {
                     constexpr auto size = std::tuple_size_v<typename std::decay_t<X>::value_type>;
                     for (size_t i = 0; i < size; ++i) {
                        if (!ensure_space(ctx, b, ix + key.size() + 32 + write_padding_bytes)) [[unlikely]] {
                           return;
                        }
