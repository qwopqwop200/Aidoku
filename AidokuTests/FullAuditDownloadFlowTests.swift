...
struct FullAuditDownloadFlowTests {
    @Test(.enabled(if: FileManager.default.fileExists(atPath:
        URL.documentsDirectory.appendingPathComponent("FullAuditDownloadFlow/enabled").path)))
    func komgaToDownloadedOfflineDisplay() async throws {
#if targetEnvironment(simulator)
        let root = URL.documentsDirectory.appendingPathComponent("FullAuditDownloadFlow")
    ...
    private func pageData(_ page: AidokuRunner.Page) throws -> Data {
        ...
        case .zipFile(let url, let path):
            ...
            return bytes
        default:
            throw NSError(domain: "FullAuditDownloadFlow", code: 1)
        }
    }
}
