...
enum ReaderJapaneseSFXFilter {
    ...
    static let speechSensitiveCandidates = loadCandidates("JapaneseSFXSpeechSensitive")
    ...
    static func apply(_ regions: [ReaderTranslationRegion], settings: ReaderTranslationSettings,
                      preparingImageEvidence: Bool = false) -> [ReaderTranslationRegion] {
        ...
        if settings.filterJapaneseSFXContext {
            return filter(regions, candidates: expandedCandidates, contextualCandidates: contextCandidates,
                          preparingImageEvidence: preparingImageEvidence)
        }
        return filter(regions, candidates: candidates, preparingImageEvidence: preparingImageEvidence)
    }

    private struct Features {
        let candidate: Bool
        let contextual: Bool
        let speechSensitive: Bool
        let kanaOnly: Bool
        let japanese: Bool
        let count: Int
        let size: CGFloat
    ...
    static func filter(
        ...
        preparingImageEvidence: Bool = false
        ...
        let features = regions.map { region in
            ...
            // Compatibility folding expands a single printed ellipsis to three periods.
            // Font geometry must use printed graphemes, not the lookup representation.
            let count = region.source.filter { !$0.isWhitespace }.count
            return Features(candidate: candidates.contains(key), contextual: contextualCandidates.contains(key),
                            speechSensitive: speechSensitiveCandidates.contains(key),
        ...
        return regions.indices.compactMap { index in
            ...
            // The more sensitive route requires an actual image sample. Missing metadata,
            // cancellation and the sampling cap must never masquerade as negative evidence.
            let visuallyChecked = !feature.contextual && region.confidence >= 0.85 &&
                (region.sfxEnclosedBackground == false || preparingImageEvidence)
            let ratio: CGFloat = feature.contextual ? 2.0 : (visuallyChecked ? 1.25 : 1.6)
            let confidence = feature.contextual ? 0.8 : 0.5
            guard region.sfxEnclosedBackground != true, feature.candidate, !feature.speechSensitive,
                  feature.kanaOnly, region.confidence >= confidence,
            ...
            // Large lettering can touch dialogue boxes (e.g. a door slam). Only a sampled,
            // high-confidence base candidate >= 3x both font references may bypass proximity.
            let margin = min(0.02, max(median, localMedian) * 0.5)
            let oversized = visuallyChecked && feature.size >= max(median, localMedian) * 3.0
            return nearestDialogueGap <= margin && !oversized ? region : nil
        }
    }
