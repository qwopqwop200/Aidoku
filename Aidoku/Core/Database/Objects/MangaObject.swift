...
public class MangaObject: NSManagedObject {
    ...
    func toNewManga() -> AidokuRunner.Manga {
        ...
        return AidokuRunner.Manga(
            ...
            tags: tags,
            status: AidokuRunner.PublishingStatus(rawValue: UInt8(exactly: status) ?? 0) ?? .unknown,
            contentRating: AidokuRunner.ContentRating(rawValue: UInt8(exactly: Int(nsfw) + 1) ?? 0) ?? .unknown,
            viewer: viewer,
            updateStrategy: neverUpdate ? .never : .always,
